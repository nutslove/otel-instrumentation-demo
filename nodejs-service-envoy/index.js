...
app.post('/inventory/reserve', async (req, res) => {
  ...
  // Call Go service for pricing (via Envoy sidecar on port 10000)
  try {
    const response = await axios.post('http://go-service:10000/pricing/calculate', {
      product_name,
      quantity,
    });
...
app.post('/inventory/reserve/error', async (req, res) => {
  ...
  // Call Go service for pricing with error endpoint (via Envoy sidecar on port 10000)
  try {
    const response = await axios.post('http://go-service:10000/pricing/calculate/error', {
      product_name,
      quantity,
    });
    const pricingResult = response.data;
